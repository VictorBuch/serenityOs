...
mkApp {
  _file = toString ./.;
  name = "deluge-vpn";
  description = "Deluge with VPN through Gluetun container";
  packages = pkgs: [];  # No packages for services

  extraConfig = { config, lib, ... }:
    let
      # Get the deluge-vpn config from the auto-generated option path
      cfg = lib.attrByPath (lib.splitString "." "apps.homelab.oci-containers.deluge-vpn") {} config;
    in
    {
      # Add custom options to the deluge-vpn namespace
      options = lib.setAttrByPath
        (lib.splitString "." "apps.homelab.oci-containers.deluge-vpn")
        delugeVpnOptions;
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf (cfg.enable && cfg.mam-dynamic-seedbox.enable or false) {
          ...
          systemd.timers.mam-ip-update = {
            ...
            timerConfig = {
              OnBootSec = "5min";
              OnUnitActiveSec = cfg.mam-dynamic-seedbox.interval or "30min";
              Persistent = true;
            };
          };
