...
}:
{
  ...
  isoImage = {
    ...
    # ISO name
    isoName = lib.mkForce "nixos-shepherd-arm-${config.system.nixos.label}-${pkgs.stdenv.hostPlatform.system}.iso";

    # Make the installer bootable on ARM systems
    makeEfiBootable = true;
    makeUsbBootable = true;
  };

  # ARM-specific boot configuration
  # Some ARM systems may need specific boot settings
